...
export async function action({ request}: ActionFunctionArgs): Promise<ActionMessage> {
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="bg-blue-600 p-6 text-white">
                    ...
                    <p className="text-blue-100 text-center mt-2">กรุณากรอกรหัสผ่านใหม่ตามเงื่อนไขที่กำหนด</p>
                </div>
                ...
                {fetcher.data?.success ? (
                    <div className="p-8">
                        <div className="flex flex-col items-center justify-center text-center space-y-4">
                            ...
                            <h3 className="text-xl font-semibold text-gray-800">รหัสผ่านถูกแก้ไขเรียบร้อย!</h3>
                            <p className="text-gray-600">รหัสผ่านใหม่ของคุณถูกอัพเดทในระบบเรียบร้อยแล้ว ตอนนี้คุณสามารถ Login ด้วยรหัสใหม่ได้แล้ว.</p>
                            <a href="/login" className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition duration-200 ease-in-out">
                                ไปที่หน้า Login
                            </a>
                        </div>
                    </div>
                    ...
                    <fetcher.Form method="post" className="p-8 space-y-6">
                        ...
                        <div className="space-y-2">
                            <div className="w-full bg-gray-200 rounded-full h-1.5">
                                <div 
                                    className={`h-1.5 rounded-full ${
                                        getPasswordStrengthPercentage() <= 20 ? 'bg-red-500' : 
                                        getPasswordStrengthPercentage() <= 60 ? 'bg-yellow-500' : 
                                        'bg-green-500'
                                    }`}
                            ...
                            <ul className="grid grid-cols-2 gap-x-4 gap-y-1">
                                <li className="flex items-center space-x-2 text-xs">
                                    <span className={`w-4 h-4 rounded-full flex items-center justify-center ${passwordStrength.length ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                                        {passwordStrength.length ? '✓' : ''}
                                    </span>
                                    <span>At least 8 characters</span>
                                </li>
                                <li className="flex items-center space-x-2 text-xs">
                                    <span className={`w-4 h-4 rounded-full flex items-center justify-center ${passwordStrength.uppercase ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                                        {passwordStrength.uppercase ? '✓' : ''}
                                    </span>
                                    <span>Uppercase letter</span>
                                </li>
                                <li className="flex items-center space-x-2 text-xs">
                                    <span className={`w-4 h-4 rounded-full flex items-center justify-center ${passwordStrength.lowercase ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                                        {passwordStrength.lowercase ? '✓' : ''}
                                    </span>
                                    <span>Lowercase letter</span>
                                </li>
                                <li className="flex items-center space-x-2 text-xs">
                                    <span className={`w-4 h-4 rounded-full flex items-center justify-center ${passwordStrength.number ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                                        {passwordStrength.number ? '✓' : ''}
                                    </span>
                                    <span>Number</span>
                                </li>
                                <li className="flex items-center space-x-2 text-xs">
                                    <span className={`w-4 h-4 rounded-full flex items-center justify-center ${passwordStrength.special ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                        ...
                        <div>
                            ...
                            {errors?.confirmPassword?.[0] && <p className="mt-1 text-sm text-red-600">{errors.confirmPassword[0]}</p>}
                        </div>
                        ...
                        >
                            {fetcher.state === "submitting" ? "Processing..." : "Reset Password"}
                        </button>
                    </fetcher.Form>
                )}
            </div>
